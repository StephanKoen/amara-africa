...
export async function downloadCFOReport(stats, orgName = 'Acme Corp') {
  ...
  function box(x, bY, w, h, hex, stroke) {
    fill(hex); draw(stroke || hex)
    pdf.rect(x, bY, w, h, stroke ? 'FD' : 'F')
  }
  ...
  ink(C.muted); setFont(8,'normal'); txt(orgName, W-M, 20, { align:'right' })
  y = 50
  ...
  // Meta dots row
  const metaItems = ['Q2 2025', `${s.totalTrips||0} trips`, `${s.uniqueTravelers||0} travelers`, s.dateRange||'Oct 2024 – Mar 2025']
  ...
  setFont(7.5,'normal')
  ink('#C4B5FD')
  txt(`${s.dateRange||''} · ${s.totalTrips||0} trips · ${s.uniqueTravelers||0} travelers · ↑ 8% vs prior quarter`, M+14, hy+33)
  ...
  rightStats.forEach(st => {
    ink(C.white); setFont(6.5,'normal'); txt(st.label, rx, hy+2)
    setFont(16,'bold'); txt(st.value, rx, hy+16)
    setFont(7,'normal'); ink('#C4B5FD'); txt(st.sub, rx, hy+27)
    rx += 56
  })
  y += heroH + 14
  ...
  cats.forEach((cat, i) => {
    ...
    txt(cat.name.slice(0,4), card1X+8, ry+5)
    // bar track
  ...
  // Pill badge
  const outOf = 100 - (s.complianceRate||0)
  const pillTxt = `${outOf}% out-of-policy spend`
  ...
  const lx = M + 10
  ink(C.navy); setFont(9,'bold'); txt('What changed this quarter', lx, y+12)
  const changes = [
    '✈  Air spend rose due to shorter booking windows on top domestic routes.',
    '🏨  Hotel compliance weakened due to non-preferred property use.',
    '⚠  Duplicate bookings and violations created direct financial risk.',
  ]
  changes.forEach((c, i) => {
    ink(C.text); setFont(7.5,'normal')
    const lines = pdf.splitTextToSize(c, narW-18)
    lines.forEach((l, li) => txt(l, lx, y + 22 + i * 18 + li * 10))
  })

  // Right column: Recommended actions
  const rx2 = M + narW + 22
  ink(C.navy); setFont(9,'bold'); txt('Recommended actions', rx2, y+12)
  const actions = [
  ...
  actions.forEach((a, i) => {
    ...
    txt('→', rx2, y + 22 + i * 19)
    ink(C.text); setFont(7.5,'normal')
    const lines = pdf.splitTextToSize(a.text, narW-22)
    lines.forEach((l, li) => txt(l, rx2+14, y + 22 + i * 19 + li * 9))
  })
  y += narH + 12
  ...
  const kpis = [
    ...
    { label:'SAVINGS',       value:'R9.2K',             trend:'22% pipeline',  tColor:C.green,  bench:fmt(s.potentialSavings) },
    { label:'COMPLIANCE',    value:fmtPct(s.complianceRate), trend:'↓ vs Q1', tColor:C.red,    bench:'Target: 90%' },
  ...
  txt('Sorted by spend · ' + (s.dateRange||''), W-M-10, y+13, { align:'right' })

  // Column headers
  const colY = y + 22
  draw('#EDE9FE'); pdf.setLineWidth(0.3); pdf.line(M, colY, M+CW, colY)
  ink(C.muted); setFont(6.5,'normal')
  txt('DEPARTMENT', M+10, colY+8)
  txt('SPEND SHARE', M+130, colY+8)
  txt('SPEND', M+320, colY+8)
  txt('COMP.', M+375, colY+8)
  txt('VS Q1', M+420, colY+8)
  ...
  depts.forEach((d, i) => {
    const ry = colY + 14 + i * 20
    if (i % 2 === 0) { fill(C.dim); pdf.rect(M+1, ry-8, CW-2, 20, 'F') }
    // Dept name
    ink(C.navy); setFont(8,'normal'); txt(d.dept, M+10, ry+3)
    // Bar track
    ...
    // Compliance %
    const deptRecs = records.filter(r=>r.department===d.dept)
  ...
  // SECTION 8 — Footer
  // ═════════════════════════════════════════════════════════════
  // Stick to bottom of page if space, else add page
  const footY = Math.max(y + 10, H - 36)
  draw(C.border); pdf.setLineWidth(0.4)
  pdf.line(M, footY, M+CW, footY)
  ink(C.muted); setFont(7,'normal')
  txt(`Last updated ${todayStr()} · ${records.length} records · Powered by Traivio`, M, footY+12)
